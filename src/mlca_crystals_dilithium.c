...
static MLCA_RC dilithium_sign(const mlca_ctx_t *ctx, unsigned char *sig, size_t *siglen, const unsigned char *m, size_t mlen, const unsigned char *sk) {
	const crystals_alg_ctx *alg_ctx = ctx->alg_impl[0]->alg_ctx;
	int rc = alg_ctx->sign(sig, *siglen, m, mlen, sk, ctx->alg[0]->encodings->encoding->crypto_secretkeybytes, (const unsigned char *)ctx->alg[0]->algorithm_oid, strlen(ctx->alg[0]->algorithm_oid));
	if (rc > 0) {
		*siglen = rc;
		return MLCA_OK;
	} else {
...
const mlca_alg_t dilithium_mlca_ctx[6] = {
	{
